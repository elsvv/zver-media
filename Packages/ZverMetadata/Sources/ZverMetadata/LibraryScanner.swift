...
public enum LibraryScanner {
    ...
    /// большинство `.m3u`/`.cue`) → **Shift-JIS ТОЛЬКО при явном японском** →
    /// **Windows-1251 ТОЛЬКО при явной кириллице** → Windows-1252 → системная
    /// определялка → Latin-1 (старые `.m3u`/западные `.cue`).
    ///
    ...
    /// случайный Latin-1→SJIS даёт ОДИНОЧНЫЕ кандзи среди ASCII (серии нет).
    ///
    /// Так же «жадна» и Windows-1251, и сам Windows-1252: как однобайтовые, они
    /// декодируют ЛЮБОЙ поток. Рус./укр. EAC-рип «весь CD одним `.flac` + `.cue`» пишет
    /// cue в 1251; без явной ветки жадный 1252 превратил бы и имя в директиве `FILE`, и
    /// названия в кракозябры — имя не совпало бы с реальным `.flac`, и образ не
    /// раскрылся бы (весь альбом — ОДИН трек). Поэтому 1251 пробуем ПЕРЕД 1252, но лишь
    /// при доказательстве кириллицы: СЕРИЯ ≥3 кириллических подряд (см. `hasCyrillicRun`).
    /// Порог 3 (а не 2, как у CJK): 1251 однобайтовая, западные акценты 1↔1 ложатся в
    /// кириллицу, поэтому рядом стоящие 0xFC 0xDF (нем. `üß`) дали бы «юя» — ложную серию
    /// из 2; реальное же кириллическое слово почти всегда ≥3 букв. Западный/UTF-8 текст
    /// в эту ветку не попадает и читается как прежде.
    ...
    static func readText(_ url: URL) -> String? {
        ...
        // Windows-1251 — только с доказательством кириллицы (иначе жадный 1252 ниже
        // прочитал бы рус./укр. cue как кракозябры; см. док выше про раскрытие образа).
        if let cyr = String(data: data, encoding: .windowsCP1251), hasCyrillicRun(cyr) { return cyr }
    ...
    /// Есть ли серия из ≥3 подряд идущих кириллических символов — признак настоящего
    /// кириллического текста (рус./укр. рип), а не случайно «успешного» 1251-декода
    /// западных байтов. Одиночные акценты (`ö`→`ц`) серии не дают; рядом стоящая пара
    /// (нем. `üß`→`юя`) даёт серию 2 — поэтому порог именно 3 (реальное кириллическое
    /// слово почти всегда ≥3 букв, а 3 подряд западных акцента в метаданных не встречаются).
    private static func hasCyrillicRun(_ s: String) -> Bool {
        var run = 0
        for scalar in s.unicodeScalars {
            switch scalar.value {
            case 0x0400...0x04FF:   // Cyrillic (вкл. рус./укр. буквы)
                run += 1
