...
///
/// MVP-вход (см. план этапа 4): владелец вставляет ~годовой OAuth-токен (права
/// `cloud_api:disk.read` + `cloud_api:disk.write` — контент в папке `zver-media`
/// в корне Диска), он ложится в Keychain (`CloudAccount`). Поле ввода
...
struct SettingsView: View {
    ...
    private var accountSection: some View {
        Section {
            ...
            if !account.isAuthorized {
                Text("Вставьте OAuth-токен Яндекс.Диска (права «Чтение всего Диска» + "
                     + "«Запись в любом месте на Диске»). Контент уходит в папку "
                     + "«zver-media» в корне Диска. Токен хранится в Keychain "
                     + "устройства и не покидает его.")
            }
        }
    }
